  ...
  async generateSessionReport(sessionState, results) {
    ...
    const report = {
      ...
      completionRate: results.totalTasks > 0 ? 
        Math.round((results.completed / results.totalTasks) * 100) : 0
    };
  ...
  calculateMetrics(sessionState, results) {
    ...
    return {
      ...
      averageTaskDuration: completedTasks.length > 0 ? 
        Math.round(completedTasks.reduce((sum, ct) => sum + (ct.result.duration || 0), 0) / completedTasks.length) : 0,
      
      tasksPerHour: duration > 0 ? 
        Math.round((results.completed / (duration / 3600000)) * 100) / 100 : 0,
      
      timeUtilization: duration > 0 ? 
        Math.round((completedTasks.reduce((sum, ct) => sum + (ct.result.duration || 0), 0) / duration) * 100) : 0,
      
      errorRate: results.totalTasks > 0 ? 
        Math.round((results.failed / results.totalTasks) * 100) : 0,
